...
- 启动应用后，右上角点击“站点与分身”按钮
- 左侧为站点中心，可新增、编辑、置顶、删除站点
- 右侧为当前站点对应的分身列表，可新增、编辑、置顶、删除分身

![站点与分身管理中心](docs/longshot20260404080511.png)

说明：

- 站点支持 `domain`、`entry_url`、`group` 三种模式
- 创建站点后会自动尝试拉取 favicon 作为站点图标
- 点击左侧站点卡片后，右侧会展示该站点下的全部分身

### 2. 打开分身并进入浏览工作台

- 在分身卡片上点击打开，即可进入对应账号分身
- 每个分身都会复用自己的独立浏览上下文
- 同一分身再次打开时，会直接聚焦到已打开标签，不会重复创建新实例

![分身工作台与标签栏](docs/longshot20260404080618.png)

说明：

- 顶部标签以“站点名称 - 分身名称”的形式展示
- 标签前会显示站点小图标，便于快速识别当前分身
- 标签支持右键操作，例如关闭当前、关闭其他、外置打开

### 3. 使用内嵌浏览器与地址栏

- 浏览区默认铺满主工作区，适合持续登录和多账号切换
- 顶部紧凑地址栏支持前进、后退、刷新、回首页
- 右上角设置抽屉为悬浮模式，不会改变浏览区布局

![内嵌浏览器使用界面](docs/longshot20260404080703.png)

说明：

- 内嵌模式与外置模式共用同一分身数据，不会产生新空白会话
- 打开管理抽屉或设置抽屉时，浏览区会保留原始内容并加遮罩
- 适合在同一软件内持续管理多个网站账号
