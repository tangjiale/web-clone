...
function isTopLevelArtifact(sourceDir, filePath) {
  return path.dirname(filePath) === sourceDir;
}
...
const platformConfig = {
  ...
  'windows-x64': [
    [
      'exe',
      (name, filePath) => isTopLevelArtifact(sourceDir, filePath) && name.endsWith('.exe'),
      `${normalizedBase}_x64-setup.exe`,
    ],
    [
      'blockmap',
      (name, filePath) =>
        isTopLevelArtifact(sourceDir, filePath) &&
        (name.endsWith('.exe.blockmap') || name.endsWith('.blockmap')),
      `${normalizedBase}_x64-setup.exe.blockmap`,
    ],
    [
      'yml',
      (name, filePath) =>
...
if (platformId === 'windows-x64') {
  const installerPath = copiedTargets.find((filePath) => filePath.endsWith('_x64-setup.exe'));
  if (!installerPath) {
    console.error('未找到 Windows 安装包 exe');
    process.exit(1);
  }
  ...
  if (installerSize < 10 * 1024 * 1024) {
    console.error(
      `Windows 安装包体积异常: ${path.basename(installerPath)} 仅 ${(installerSize / 1024).toFixed(1)} KB，疑似收集到了错误的 exe`,
    );
    process.exit(1);
  }
}
